...
def save(bundle: ModelBundle, model_dir: Path, promote: bool) -> Path:
    target = Path(model_dir) / bundle.version
    ...
    (target / "model_card.json").write_text(json.dumps(bundle.card, indent=2, ensure_ascii=False),
                                            encoding="utf-8")
    if promote:
        (Path(model_dir) / CURRENT_FILE).write_text(json.dumps({"version": bundle.version}), encoding="utf-8")
    return target
